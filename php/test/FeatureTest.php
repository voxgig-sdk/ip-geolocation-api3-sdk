...
// IpGeolocationApi3 SDK feature test
//
...
class FtCtrl extends IpGeolocationApi3Control
{
...
class FtClient
{
    ...
    private IpGeolocationApi3Utility $utility;

    public function __construct(IpGeolocationApi3Utility $utility, array $options)
    {
    ...
    public function get_utility(): IpGeolocationApi3Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpGeolocationApi3Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpGeolocationApi3Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpGeolocationApi3Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpGeolocationApi3Config::make_feature($name);
    ...
    private static function buildUrl(IpGeolocationApi3Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpGeolocationApi3Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpGeolocationApi3Response($fetched);
        }
    ...
    private function fail(IpGeolocationApi3Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpGeolocationApi3Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
