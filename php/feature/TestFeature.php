...
// IpGeolocationApi3 SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class IpGeolocationApi3TestFeature extends IpGeolocationApi3BaseFeature
{
    ...
    public function init(IpGeolocationApi3Context $ctx, array $options): void
    {
        ...
        $test_fetcher = function (IpGeolocationApi3Context $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = IpGeolocationApi3Param::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (IpGeolocationApi3Context $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(IpGeolocationApi3Context $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = IpGeolocationApi3Param::call($ctx, $k);
