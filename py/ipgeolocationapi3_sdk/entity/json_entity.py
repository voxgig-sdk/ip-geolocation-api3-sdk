...
from ipgeolocationapi3_sdk.ipgeolocationapi3_types import (
    Json,
    JsonLoadMatch,
)
...
class JsonEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return JsonEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Json:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Json().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
