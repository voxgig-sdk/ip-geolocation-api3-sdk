...
//     (\`new IpGeolocationApi3SDK(...)\` / \`IpGeolocationApi3SDK.test(...)\` become
//     \`IpGeolocationApi3SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IpGeolocationApi3SDK'
...
const TEST_SEED = {"entity":{"json":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IpGeolocationApi3SDK(...)\` and any
// \`IpGeolocationApi3SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpGeolocationApi3SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpGeolocationApi3SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
