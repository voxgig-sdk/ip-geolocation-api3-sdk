...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*IpGeolocationApi3SDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *IpGeolocationApi3Error {
	return NewIpGeolocationApi3Error(code, msg, ctx)
}
