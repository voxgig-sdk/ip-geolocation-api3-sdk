...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        IpGeolocationApi3SDK $client,
        IpGeolocationApi3Utility $utility
    ): IpGeolocationApi3Context {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new IpGeolocationApi3Context($ctxmap, null);
    ...
    private static function fixctx(IpGeolocationApi3Context $ctx, IpGeolocationApi3SDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?IpGeolocationApi3Error
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IpGeolocationApi3Error($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new IpGeolocationApi3Result([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new IpGeolocationApi3BaseFeature();
    ...
    {
        $hook_client = IpGeolocationApi3SDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends IpGeolocationApi3BaseFeature {
            public $hook_fn = null;
            public function TestHook(IpGeolocationApi3Context $ctx): void
            {
    ...
    {
        $init_client = IpGeolocationApi3SDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends IpGeolocationApi3BaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new IpGeolocationApi3SDK([
            'system' => [
    ...
    {
        $blocked_client = new IpGeolocationApi3SDK([
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new IpGeolocationApi3Spec([
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new IpGeolocationApi3Spec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = IpGeolocationApi3SDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new IpGeolocationApi3Operation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = IpGeolocationApi3SDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof IpGeolocationApi3Error) ? $result->err->msg : (string)$result->err;
