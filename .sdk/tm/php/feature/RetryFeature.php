...
// IpGeolocationApi3 SDK retry feature
...
class IpGeolocationApi3RetryFeature extends IpGeolocationApi3BaseFeature
{
    ...
    public function init(IpGeolocationApi3Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpGeolocationApi3Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpGeolocationApi3Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpGeolocationApi3Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
