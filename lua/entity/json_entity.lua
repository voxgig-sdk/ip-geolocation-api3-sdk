...
local JsonEntity = {}
JsonEntity.__index = JsonEntity


function JsonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, JsonEntity)
  self._name = "json"
  self._client = client
...
function JsonEntity:get_name()
  return self._name
end


function JsonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return JsonEntity.new(self._client, opts)
end
...
function JsonEntity:mark_deleted()
  self._deleted = true
end


function JsonEntity:deleted()
  return true == self._deleted
end


function JsonEntity:data_set(args)
  if args ~= nil then
...
function JsonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function JsonEntity:match_set(args)
  if args ~= nil then
...
function JsonEntity:stream(action, args, callopts)
...
---@param reqmatch JsonLoadMatch
---@param ctrl? table
---@return Json
---@return string? err
function JsonEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function JsonEntity:_run_op(ctx, post_done)
  local utility = self._utility
