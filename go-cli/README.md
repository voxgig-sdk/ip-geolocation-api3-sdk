# ip-geolocation-api3-cli

boru-driven command-line client **and** interactive REPL for the IpGeolocationApi3
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-geolocation-api3-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-geolocation-api3-cli --help

# 3. Provide credentials once, via the environment
export IP_GEOLOCATION_API3_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-geolocation-api3-cli load 1 json            # {id:1} shorthand
./ip-geolocation-api3-cli load '{id:1}' json       # explicit match map

# 5. Override the API base URL for a single call
IP_GEOLOCATION_API3_BASE=https://api.example.com ./ip-geolocation-api3-cli load 1 json

# 6. No arguments -> interactive REPL
./ip-geolocation-api3-cli
ip-geolocation-api3> load 1 json
ip-geolocation-api3> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-geolocation-api3-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-geolocation-api3-cli load 1 json
   ```
...
```sh
./ip-geolocation-api3-cli load 1 json          # scalar shorthand for {id:1}
./ip-geolocation-api3-cli load '{id:1}' json     # explicit match map
```
...
./ip-geolocation-api3-cli load 1 json
```
...
Run with no arguments to open a REPL (prompt `ip-geolocation-api3>`). Each line is
evaluated as its own boru expression:

```text
$ ./ip-geolocation-api3-cli
ip-geolocation-api3> load 1 json
ip-geolocation-api3> /help
ip-geolocation-api3> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `json`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-geolocation-api3-cli`. |
...
(`./ip-geolocation-api3-cli <expr>`) and interactively (the REPL), and expressions compose the
