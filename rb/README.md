# IpGeolocationApi3 Ruby SDK



The Ruby SDK for the IpGeolocationApi3 API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Json` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = IpGeolocationApi3SDK.test(testopts, sdkopts)
```
...
raise a `IpGeolocationApi3Error` (a `StandardError` subclass), so wrap
...
```
rb/
├── IpGeolocationApi3_sdk.rb       -- Main SDK module
...
The main module (`IpGeolocationApi3_sdk`) exports the SDK class
