...
module IpGeolocationApi3Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpGeolocationApi3Response.new({})
    ...
    if fetch_err
      ...
      response = IpGeolocationApi3Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpGeolocationApi3Response.new(fetched)
    else
