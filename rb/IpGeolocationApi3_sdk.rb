...
require_relative 'IpGeolocationApi3_types'


class IpGeolocationApi3SDK
  ...
  def initialize(options = {})
    ...
    utility = IpGeolocationApi3Utility.new
    @_utility = utility
    ...
    feature_opts = IpGeolocationApi3Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IpGeolocationApi3Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IpGeolocationApi3Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IpGeolocationApi3Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IpGeolocationApi3Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IpGeolocationApi3Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IpGeolocationApi3Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => IpGeolocationApi3Error.new(
        "#{op}_allow",
        "IpGeolocationApi3SDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue IpGeolocationApi3Error => err
    ...
    if fetched.is_a?(Hash)
      status = IpGeolocationApi3Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IpGeolocationApi3Error.new(
        "graphql_error", "IpGeolocationApi3SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = IpGeolocationApi3SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
