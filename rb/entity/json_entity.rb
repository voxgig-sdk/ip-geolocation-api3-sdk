...
class JsonEntity
  ...
  def make
    opts = @_entopts.dup
    JsonEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpGeolocationApi3Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Json, Hash] the current Json data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpGeolocationApi3Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Json fields)
  def match_get
  ...
  # Load a single Json.
  #
  # @param reqmatch [JsonLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Json.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Json, Hash] the loaded Json; raises IpGeolocationApi3Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpGeolocationApi3Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
