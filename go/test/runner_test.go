...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("IPGEOLOCATIONAPI3_TEST_LIVE") == "TRUE" ||
		os.Getenv("IPGEOLOCATIONAPI3_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("IPGEOLOCATIONAPI3_TEST_EXPLAIN"); explain != "" {
